...
export default {
  ...
  methods: {
    ...
    makeClone(key) {
      var deep = this.elements[key].attributes["hero-deep"]
    ...
    checkClones() {
      if (!this.$hero.items[this.tag]) return;
      for (var key in this.$hero.items) {
        if (key == this.tag) continue;
        var elements = this.$hero.items[key];
  ...
  destroyed(){
    setTimeout(()=>{
    for(var tag in this.$hero.items){
      for(var key in this.$hero.items[tag]){
  ...
  mounted() {
    this.$nextTick(function() {
      ...
      for (var key in this.$scopedSlots.default()[0].context.$refs) {
        this.$hero.inUse[this.tag][key] = true;
        var ref = this.$scopedSlots.default()[0].context.$refs[key];
        if (Array.isArray(ref)) {
          for (var i = 0; i < ref.length; i++) {
            var isScoped = ref[i].attributes["hero-tag"]
